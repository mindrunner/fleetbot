...
export const getCargoType = (cargoTypes: Array<CargoType>, game: Game, mint: PublicKey) : CargoType => {
    const cargoType = cargoTypes.find(ct =>
        game.data.cargo.statsDefinition.equals(ct.data.statsDefinition) &&
        mint.equals(ct.data.mint)
    )

    if(!cargoType) {throw new Error(`Cargo type not found for mint ${mint}.`)}
