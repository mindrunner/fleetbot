...
export const getCargoPodsForStarbasePlayer =
    async (starbasePlayer: StarbasePlayer, programs: StarAtlasPrograms): Promise<CargoPod> => {
        ...
        if (cargoPod.type === 'error') {throw new Error('Error reading cargoPods account')}

        return cargoPod.data
    }

export const getStarbasePlayer =
    async (player: Player, starbase: Starbase, programs: StarAtlasPrograms): Promise<StarbasePlayer> => {
        ...
        if (!starbasePlayer) {
            ...
            const instructionReturns = [
                ...
                StarbasePlayer.createCargoPod(
                    ...
                    {
                        keyIndex: 0,
                        podSeeds: Array.from(Keypair.generate().publicKey.toBuffer())
                    }
                )
            ]

            logger.warn('Starbase player not found, creating', { player: player.profile.key.toBase58(), starbase: starbase.key.toBase58() })
            await sendAndConfirmInstructions(await ixReturnsToIxs(instructionReturns, player.signer))

            return getStarbasePlayer(player, starbase, programs)
        }

        if (starbasePlayer.type === 'error') {throw new Error('Error reading starbasePlayer account')}
