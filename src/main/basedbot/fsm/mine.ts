...
const transition = async (fleetInfo: FleetInfo, player: Player, config: MineConfig): Promise<void> => {
    const cargoLoad = player.cargoTypes.filter(ct => !ct.data.mint.equals(player.game.data.mints.food))
        .reduce((acc, cargoType) => {
            const load = fleetInfo.cargoLevels.cargo.get(cargoType.data.mint.toBase58()) ?? 0
    ...
    const hasEnoughAmmo = cargoLevelAmmo >= fleetInfo.cargoStats.ammoCapacity - 100
    const hasEnoughFuel = cargoLevelFuel >= fleetInfo.cargoStats.fuelCapacity - 100
    ...
    switch (fleetInfo.fleetState.type) {
        case 'Idle': {
            logger.info(`${fleetName} is idle at ${fleetInfo.fleetState.data.sector} [Starbase: ${currentStarbase ? getName(currentStarbase) : 'N/A'}]`)

            if(!currentStarbase && cargoLevelFuel < 1) {
                logger.warn(`Fleet: ${fleetName} is out of fuel and not at a starbase, need self destruction`)
            ...
            if (isAtHomeBase) {
                ...
                if (hasCargo) {
                    logger.info(`Fleet: ${fleetName} has ${cargoLoad} ${resourceName}, docking to unload`)

                    return dock(fleetInfo, location, player)
                }
                if (!hasEnoughFood || !hasEnoughFuel || !hasEnoughAmmo) {
                    logger.info(`Fleet: ${fleetName} doesn't have enough resources, docking to resupply`)

                    return dock(fleetInfo, location, player)
                }
                if (isSameBase) {
                    logger.info(`Fleet: ${fleetName} is at home/target base, mining`)

                    return mine(fleetInfo, player, resource)
                }
                logger.info(`Fleet: ${fleetName} is at home base, moving to target base`)
            ...
            if (isAtTargetBase && !isSameBase) {
                ...
                if (hasCargo) {
                    logger.info(`Fleet: ${fleetName} has ${cargoLoad} ${resourceName}, returning home`)
                ...
                logger.info(`Fleet: ${fleetName} doesn't have enough food, returning home`)
            ...
            return move(fleetInfo, hasCargo || !hasEnoughFood ? homeBase : targetBase, player, warpMode)
        }
        case 'StarbaseLoadingBay': {
            ...
            if (hasCargo) {
                logger.info(`Fleet: ${fleetInfo.fleetName} has ${cargoLoad} cargo, unloading`)

                return unloadAllCargo(fleetInfo, fleetInfo.location, player, fleetInfo.fleet.data.cargoHold)
            }
            ...
            if (!hasEnoughFood) {
                logger.info(`Fleet: ${fleetInfo.fleetName} is loading ${desiredFood - cargoLevelFood} food`)
        ...
        case 'Respawn': {
            ...
            if (ETA.isBefore(now())) {
                ...
                logger.info(`Fleet: ${fleetInfo.fleetName} respawning at ${fleetInfo.fleetState.data.sector}. ETA: ${dayjs.duration(ETA.diff(now())).humanize(false)}. Destruction time: ${destructionTime}`)
            }
            break
        }
        default:
...
export const createMiningStrategy = (miningConfig: MineConfig, p: Player): Strategy => {
    const config = miningConfig
    const player = p

    return {
        send: (fleetInfo: FleetInfo): Promise<void> => transition(fleetInfo, player, config)
    }
}
